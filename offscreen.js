...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg.action !== 'OFFSCREEN_DOWNLOAD' && msg.action !== 'offscreen_download') {
    return;
  }
  ...
  (async () => {
    try {
      ...
      if (msg.bytes && Array.isArray(msg.bytes)) {
        // ArrayBuffer 방식 (content.js 경로)
        const u8 = new Uint8Array(msg.bytes);
        blob = new Blob([u8], { type: msg.mimeType || 'image/png' });
      } else if (msg.url) {
        // URL 방식 (fallback)
